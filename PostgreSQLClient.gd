...
func execute(sql: String) -> Array:
	if status == Status.STATUS_CONNECTED:
		var _unused
		var request := request('Q', sql.to_utf8_buffer() + PackedByteArray([0]))
		
		if stream_peer_tls.get_status() == StreamPeerTLS.STATUS_CONNECTED:
			stream_peer_tls.put_data(request)
		else:
			_unused = peer.put_data(request)
		
		while client.get_status() == StreamPeerTLS.STATUS_CONNECTED and status == Status.STATUS_CONNECTED:
			var reponce := [OK, PackedByteArray()]
			
			if stream_peer_tls.get_status() == StreamPeerTLS.STATUS_CONNECTED:
				stream_peer_tls.poll()
				if stream_peer_tls.get_available_bytes():
					reponce = stream_peer_tls.get_data(stream_peer_tls.get_available_bytes()) # I don't know why it crashes when this value (stream_peer_ssl.get_available_bytes()) is equal to 0 so I pass it a condition. It is probably a Godot bug.
				else:
					continue
			else:
